...
| `test-oneonone.mjs` | The one-on-one stops distinguishing "we met" from "I asked" from "I haven't asked", giving one gets confused with getting one, an ask reaches somebody other than that person's one approved mentor, the two-a-month count stops being monthly or stops being undoable, or a staff-debt amount leaks out of the anonymous base total |
...
| `test-report.mjs` | The report stops being built from the app's own weekly numbers — a figure somebody typed on the Programs tab overriding what the ministries logged — or it stops matching the report actually filed — a wrong percentage, a lost team, the year-to-date measured against one period, a semester that is no longer its two quarters added together, or a typed field breaking out into markup |
| `test-programs-ui.mjs` | The Programs tab starts asking anyone to retype a figure their ministry already logs, the mismatch warning stops firing, an annual estimate stops belonging to its ministry, the forms stop being built from `programs.js`, a field the report needs is not sent, records get fetched at boot instead of on opening the tab, a record can be saved against a period wider than one quarter or an edit moves a row's quarter, the report sheet stops being modal or stops offering a way out to Google Docs, or a fourth tab breaks the 320px nav |
...
| `test-base.mjs` | The Base tab, or the weekly health form — including an untouched 1-10 slider counting as an answer, which would fill the base average with accidental fives |
| `test-mentor-health.mjs` | A mentor can no longer see their mentee by name, the conversation cues stop following that person's actual answers, or the "one-on-one finished" button disappears |
