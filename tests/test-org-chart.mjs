...
ok('the only fetch is for this quarter’s own snapshot note, once', sent.filter(f => f === 'getStructure').length === 1);
...
ok('and draws the chart from it: the person who has since left is there, crowned as Cafe leader', /Snapshot saved/.test(q1.note) && /👑 Kiri Left/.test(q1.body), q1.note);
...
ok('a quarter nobody saved shows the latest earlier snapshot and says so', /Nothing saved for this quarter — showing the Q1/.test(q2), q2);
await page.click('[data-structq="1"]');
...
await page.waitForTimeout(400);
ok('an admin sees Save this quarter’s snapshot', await page.evaluate(() => /Save this quarter/.test((document.querySelector('#structSaveBtn') || {}).textContent || '')));
await page.click('#structSaveBtn');
await page.waitForTimeout(600);
ok('Save posts the campus, year and quarter — the server builds the snapshot itself', sent.includes('saveStructure') && await page.evaluate(() => /Snapshot saved/.test(document.body.innerText) && /Update this quarter/.test(document.querySelector('#structSaveBtn').textContent)));
ok('no page errors', errors.length === 0, errors.join(' | '));
